...
def save_info(name, weight_total, img, price)
  weight_total.each_with_index do |weight, index|
...
def product(html_body)
  ...
  weight_total = html_body.xpath('//span[@class="radio_label"]')
  price = html_body.xpath('//span[@class="price_comb"]')
  img = html_body.xpath('//img[@id="bigpic"]/@src')

  save_info(name, weight_total, img, price)
end
